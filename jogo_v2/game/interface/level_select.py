...
class LevelSelect:
    ...
    def draw_grid(self):
        ...
        for i, level in enumerate(self.levels):
            ...
            color = self.dark if level["unlocked"] else (20, 20, 20)
            ...
            pygame.draw.rect(self.screen, (80, 80, 80), (x + 10, y + 50, card_w - 20, 60))
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_RIGHT and (self.selected % self.cols) < self.cols - 1:
                        self.selected += 1

                    if event.key == pygame.K_LEFT and (self.selected % self.cols) > 0:
                        self.selected -= 1

                    if event.key == pygame.K_DOWN and self.selected + self.cols < len(self.levels):
                        self.selected += self.cols

                    if event.key == pygame.K_UP and self.selected - self.cols >= 0:
                        self.selected -= self.cols

                    if event.key == pygame.K_RETURN and self.levels[self.selected]["unlocked"]:
